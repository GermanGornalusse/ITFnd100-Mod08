...
list_of_product_objects = []  # Captures the data from a file into  a list of rows
...
class FileProcessor:
    ...
    def save_data_to_file(file_name: str, list_of_product_objects: list):
        """ Writes data from a list of  rows into a file
...
class IO:
    ...
    def print_menu_tasks():
        ...
        print('''
            Menu of Options
            1) Show current data
            2) Add  a new product (to the list of product objects)
            3) Save Data to File 
            4) Exit Program       
            ''')
...
while (True):
    ...
    elif choice.strip () == '3':  # let user save current data to file
        list_of_product_objects = FileProcessor.save_data_to_file(file_name, list_of_product_objects)
        IO.print_current_products_in_list(list_of_product_objects)
        continue

    elif choice.strip () == '4':   # exit the program
        print("Good bye!")
        break